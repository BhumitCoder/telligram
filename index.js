...
bot.on('message', async (msg) => {
  ...
  if (text && !msg.photo) {
    if (text.includes('create an image') || text.includes('generate a picture') || 
      ...
      try {
        ...
        try {
          await bot.sendMessage(chatId, `Error: Failed to generate image. Please try again or simplify your description. (${error.message})`);
        } catch (sendError) {
          console.error(`Error sending image generation error message to chat ${chatId}: ${sendError.message}`);
        }
      }
    } else {
      try {
        ...
        console.log(`Received API response for prompt "${text}" in chat ${chatId}: ${textResponse}`);
        ...
        try {
          ...
          console.error(`Error sending text generation fallback to chat ${chatId}: ${sendError.message}`);
        }
      }
    }
  }
  ...
  if (msg.photo) {
    ...
    try {
      ...
      console.log(`Received image analysis response for caption "${caption}" in chat ${chatId}: ${text}`);
      await bot.sendMessage(chatId, `${text}`);
    } catch (error) {
      console.error(`Image analysis error for caption "${caption}" in chat ${chatId}: ${error.message}`);
      try {
        await bot.sendMessage(chatId, `Sorry, the API is currently slow or unavailable. Please try sending the image again later.`);
      } catch (sendError) {
        console.error(`Error sending image analysis fallback to chat ${chatId}: ${sendError.message}`);
      }
    }
  }
});
