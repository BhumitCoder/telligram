...
const testPollinationsApi = async () => {
  ...
  try {
    await axiosWithRetry({
      ...
      timeout: 10000 // 10 seconds timeout for startup test
    });
...
bot.on('message', async (msg) => {
  ...
  if (text && !msg.photo) {
    if (text.includes('create an image') || text.includes('generate a picture') || 
      ...
      try {
        ...
        console.error(`Image generation error for prompt "${text}": ${error.message}`);
      ...
      try {
        ...
        console.log(`Sending API request for prompt: ${text}`);
        ...
        console.error(`Text generation error for prompt "${text}": ${error.message}`);
  ...
  if (msg.photo) {
    ...
    try {
      ...
      try {
        console.log(`Sending image analysis request for caption: ${caption}`);
        const response = await axiosWithRetry({
          method: 'post',
          url: TEXT_API,
          data: {
        ...
        console.error(`Image analysis error for caption "${caption}": ${error.message}`);
        bot.sendMessage(chatId, `Sorry, the API is currently slow or unavailable. Please try sending the image again later.`);
      }
    } catch (error) {
